...
SELECT email, first_name, last_name FROM salespeople WHERE region IN ('Northwest', 'Southwest')
OR last_name LIKE 'M%';
...
SELECT id, status, order_total FROM orders WHERE (SELECT id FROM customers WHERE
email = 'pclark74@gmail.com') = customer_id;
